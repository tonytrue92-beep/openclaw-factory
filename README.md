...
**Главное меню (с 2026.06.11 — только боевые пункты):**

| Вариант | Что делает |
|---------|-----------|
| `1` Установить OpenClaw *(Enter)* | Движок + Telegram-бот + первый ассистент; по платному токену сразу доустановит AI-команду |
| `2` Установить AI-команду агентов | Для тех, у кого OpenClaw уже стоит — сразу качает установщик агентов |
| `3` VPS 24/7 | Инструкция разворачивания на сервере |

Демо и симуляция из меню убраны (клиентов путали); для отладки остались флаги `--dry-run` и внутренние ветки.
