...
## 2026-06-18 — Ретраи в ip_dl (РФ-origin без CDN моргает)

- **`ip_dl` теперь 3 попытки** (`--connect-timeout 10 --max-time 25`, пауза 2с)
  вместо одной. РФ-origin без CDN иногда даёт curl 28; без ретраев один блип
  терял хелпер/файл чейна. Совпадает с agents-pack ip_dl. Defense-in-depth к
  Cloudflare-проксированию (переживаем редкие блипы даже с CF).
- smoke: ассерт наличия ретраев + github-ветка переписана на busybox-safe `grep -F`.
...
- add-codex: экран ожиданий перед входом в ChatGPT (localhost/код), совет «нужен ChatGPT Pro», авто-подтверждение «модель не в списке» при вызове switch-model.
