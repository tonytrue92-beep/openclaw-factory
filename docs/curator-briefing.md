...
> 📍 **Единый контекст по ВСЕМ трём трекам (тест-драйв / движок / агенты) —
> в `openclaw-agents-pack/docs/curator-guide.md`** (актуально на 2026-06-06).
> Этот файл — про движок `openclaw-factory` детально; сверяйся с мастером.
> Свежее: `openclaw: command not found` после установки чинится через nvm
> (см. §5.1 мастер-инструкции), шаг «мозги» — opencode-ключ (Codex — опц. апгрейд).
...
### Фаза 2. Главное меню (с 2026.06.11 — 3 боевых пункта)

- **1 — Установить OpenClaw** (Enter, по умолчанию) — реальная установка (R0–R6); по платному токену в конце сама доустановит AI-команду
- **2 — Установить AI-команду агентов** — движок уже стоит, сразу качает установщик агентов
- **3 — VPS 24/7** — печатает инструкцию для сервера и выходит

Демо/симуляция из меню убраны (2026-06-11, решение Антона) — клиенты запускали их вместо установки.
