...
# ─── ПИН ВЕРСИИ OpenClaw ──────────────────────────────────────────────────
# Ставим КОНКРЕТНУЮ версию, НЕ @latest. Иначе каждый апстрим-релиз (напр.
# 2026.6.6 включил gateway.auth.mode=token → device-identity; 2026.6.x
# переименовал opencode→opencode-go) прилетает клиентам автоматом и ломает
# установки. 2026.6.6 — текущая заточенная под наш установщик (opencode-go +
# обход auth.mode none). БАМП ТОЛЬКО ВРУЧНУЮ: проверить новую версию на тест-
# машине → поменять эту строку → smoke → релиз. Единая точка правки.
...
# COURSE_TOKEN экспортируем РАНО: export-атрибут в bash сохраняется при всех
# последующих присваиваниях (--course-token / интерактив / acquire_*), поэтому
# дочерний agents.sh в `eval "bash /tmp/agents.sh"` всегда наследует токен через
# env — даже если «Полный сброс» снёс кэш ~/.openclaw/course-token. Без этого
# _ip_token в дочернем процессе пуст → gateway 401 → ложная ошибка «GitHub raw 404».
export COURSE_TOKEN
# IP_BASE объявляем РАНО (set -u): _fetch_agents_installer читает его задолго до
...
pause() {
  ...
  # EOF/Ctrl+D на паузе не должен валить весь установщик под `set -e`.
  # Реальный кейс 2026-06-15: клиент получил debug-bundle с line=3990,
  # а установка остановилась до Telegram/агентов только из-за неуспешного read.
  read -r _pause_dummy || true
  unset _pause_dummy
}

read_optional() {
  # Usage: read_optional VAR [default]
  # Bash 3.2 compatible (macOS): no nameref.
  # Returns 0 when a line was read, 1 on EOF; in both cases VAR is set.
  local __var="$1"
  local __default="${2:-}"
  local __value=""
...
require_course_token_before_real_install() {
  ...
  if [[ -z "$machine_tg_id" ]]; then
    ...
    read_optional machine_tg_id "" || true
...
install_homebrew() {
  ...
  if ! is_macos_admin; then
    ...
    if [[ $admin_status -eq 1 ]]; then
      ...
      read_optional skip_brew "y" || true
...
prompt_install_homebrew() {
  ...
  read_optional install_brew "y" || true
...
on_installer_error() {
  ...
  echo -e "   ${DIM}Команда: ${failed_cmd}${NC}"
  echo ""
  echo -e "   ${DIM}Собираю debug-bundle для саппорта...${NC}"
  collect_debug_bundle "error exit=${exit_code} at line ${line_no}; cmd=${failed_cmd}" || true
  return $exit_code
}
...
ensure_telemetry_consent() {
  ...
  tel_consent=""
  read_optional tel_consent "" || true
...
show_vps_guide() {
  ...
  echo ""
  echo -e "   ${DIM}Лучше всего — возьмите ГОТОВУЮ команду из ${BOLD}@AITeamVIPBot${NC}${DIM} (токен уже внутри)${NC}"
  echo -e "   ${DIM}и допишите ${BOLD}--vps${NC}${DIM} в конце. Если вписываете токен сами —${NC}"
  echo -e "   ${BOLD}${YELLOW}вставляйте его БЕЗ угловых скобок < > и без кавычек:${NC}"
  echo ""
  echo -e "   ${GREEN}${BOLD}COURSE_TOKEN=ВАШ_ТОКЕН IP_BASE=https://api.tonytrue.pro/ip bash -c 'bash <(curl${NC}"
...
#      Фикс:   `sudo npm install -g openclaw@${OPENCLAW_VERSION}`
#
...
diagnose_npm_eacces() {
  ...
  if [[ "$case_global" == true ]]; then
    ...
    echo -e "      ${GREEN}sudo npm install -g openclaw@${OPENCLAW_VERSION}${NC}"
    ...
    echo -e "      ${GREEN}nvm install 22 && nvm use 22 && npm install -g openclaw@${OPENCLAW_VERSION}${NC}"
    echo ""
  fi
...
install_openclaw_npm() {
  ...
  while [[ $attempt -le $max_attempts ]]; do
    ...
    npm install -g openclaw@${OPENCLAW_VERSION} 2>"$err_log" | tail -12 | while IFS= read -r line; do
      echo -e "   ${DIM}${line}${NC}"
    done
    rc=${PIPESTATUS[0]}
    set -e
...
prompt_install_node() {
  ...
  read_optional autoinstall "y" || true
...
LOGO
  ...
  echo ""
  read_optional INITIAL_CHOICE "" || true
...
echo -e "   ${DIM}┌─ 📋 скопируйте эту команду (без \$) ─────────────────────┐${NC}"
echo -e "   ${DIM}│${NC} ${YELLOW}\$${NC} ${GREEN}${BOLD}npm install -g openclaw@${OPENCLAW_VERSION}${NC}"
echo -e "   ${DIM}└──────────────────────────────────────────────────────────┘${NC}"
echo ""
...
echo -e "   ${GREEN}   npm install -g openclaw@${OPENCLAW_VERSION}${NC}   ${DIM}← вот это${NC}"
echo ""
echo -e "   ${RED}✗${NC} ${DIM}Неправильно: ${NC}${RED}\$ npm install -g openclaw@${OPENCLAW_VERSION}${NC}   ${DIM}(с долларом)${NC}"
echo -e "   ${GREEN}✓${NC} ${DIM}Правильно:   ${NC}${GREEN}npm install -g openclaw@${OPENCLAW_VERSION}${NC}     ${DIM}(без доллара)${NC}"
echo ""
...
show_cmd "npm install -g openclaw@${OPENCLAW_VERSION}"
echo ""
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  if [[ "$OPENCLAW_INSTALLED" == true ]]; then
    ...
    if [[ $install_rc -eq 0 ]]; then
      ...
      ru "  4. Или вручную позже: npm install -g openclaw@${OPENCLAW_VERSION}"
      exit 1
    fi
  fi
fi
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  if [[ -f "$HOME/.openclaw/openclaw.json" ]]; then
    ...
    read_optional RECONFIG_CHOICE "1" || true
...
PYEOF
      ...
      4)
        ...
        confirm_reset=""
        read_optional confirm_reset "" || true
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  echo ""
  read_optional BOT_TOKEN "" || true

  if [[ -z "$BOT_TOKEN" ]]; then
    echo ""
    warn "Токен Telegram-бота не введён — без него установка не будет рабочей."
    echo -e "   ${DIM}Создайте бота в @BotFather, скопируйте токен и запустите эту же команду ещё раз.${NC}"
    echo -e "   ${DIM}Если OpenClaw уже стоит, можно подключить вручную:${NC}"
...
if [[ "$DRY_RUN" == true ]]; then
  ...
  echo ""
  read_optional AGENT_ID "assistant" || true
...
[[ -n "${IP_BASE:-}" ]] && export IP_BASE   # IP_BASE уже объявлен в начале скрипта
_ip_token() {
...
PYEOF
  ...
  # Предложим открыть dashboard прямо сейчас на macOS
    ...
    read_optional _open_dash "y" || true
