...
if command -v node &>/dev/null; then
  ...
  echo ""
  ru "Node.js не найден — но это не проблема, я могу поставить его прямо сейчас."
  prompt_install_node
  # После установки перечитываем версию для красивого вывода
  if command -v node &>/dev/null; then
    NODE_VER=$(node -v)
    echo ""
    terminal "${NODE_VER}"
    ru "Теперь Node.js ${NODE_VER} установлен и готов к работе."
  fi
fi
...
if command -v npm &>/dev/null; then
  ...
  echo ""
  ru "npm не найден — устанавливаю Node.js (npm идёт в комплекте)."
  prompt_install_node
fi
...
if command -v openclaw &>/dev/null; then
  ...
  ru "OpenClaw у вас уже установлен (${OC_VER}) — пропускаем этот шаг."
  echo ""
else
...
if command -v openclaw &>/dev/null; then
  ...
  echo ""
  warn "OpenClaw не найден после установки. Попробуйте открыть новый терминал."
  ru "Иногда нужно перезапустить терминал, чтобы PATH обновился."
fi
