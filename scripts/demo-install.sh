...
_HEARTBEAT_TIPS=(
  ...
  "💡 По умолчанию — бесплатная MiniMax 2.5 Free. Сменить на Claude или GPT: openclaw-switch-model"
  "💡 Личность агента — в файлах IDENTITY.md и AGENTS.md. Меняете тон, стиль, правила в обычном текстовом редакторе."
  "💡 Сессии кэшируются — бот помнит разговор. Начать с чистого листа: openclaw sessions cleanup"
  "💡 Dashboard работает на http://127.0.0.1:18789 — все каналы, агенты, последние сообщения."
  "💡 Все конфиги в ~/.openclaw/ — бэкапится как обычная папка (Dropbox, iCloud, rsync — что угодно)."
)
...
ensure_telemetry_consent() {
  ...
  echo -e "   ${DIM}Помогает нам улучшать установщик: видим, на каком шаге бросают,${NC}"
  echo -e "   ${DIM}какие ОС чаще встречают ошибки. Имена, ключи, IP — не собираем.${NC}"
  echo -e "   ${DIM}Пока endpoint не задеплоен — всё пишется ТОЛЬКО локально в:${NC}"
  echo -e "   ${DIM}   ${TELEMETRY_LOG}${NC}"
  echo -e "   ${DIM}Можно посмотреть/удалить в любой момент.${NC}"
  echo ""
  ...
  if [[ "$tel_consent" == "y" || "$tel_consent" == "Y" ]]; then
    ...
    echo -e "   ${DIM}Окей, не собираем. Можно изменить: echo yes > ${TELEMETRY_CONSENT_FILE}${NC}"
  fi
  echo ""
}
...
run_diagnostics() {
  ...
  if [[ ! -f "$HOME/.openclaw/openclaw.json" ]]; then
    ...
    if command -v openclaw &>/dev/null; then
      ...
      if [[ "$gw_mode" == "local" ]]; then
        ...
        echo -e "   ${RED}✗${NC} gateway.mode='${gw_mode:-не задан}' (должен быть 'local')"
        issues+=("gateway.mode не local — бот не поднимется")
      fi
  ...
  if [[ ${#issues[@]} -eq 0 ]]; then
    ...
    echo -e "   ${CYAN}→${NC} если gateway мёртв — ${GREEN}openclaw config set gateway.mode local && openclaw gateway restart${NC}"
  fi
  echo ""
}
