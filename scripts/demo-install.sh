...
if [[ "$DRY_RUN" == true ]]; then
  ...
  if [[ ! -f "$HOME/.openclaw/openclaw.json" || "${NEED_KEY_INPUT:-false}" == true ]]; then
    ...
    explain "Что нужно от вас — аккаунт ChatGPT:" \
      ...
      "" \
      "Вход происходит на сайте OpenAI — мы НЕ видим ваш пароль." \
      "Нет аккаунта? Создайте бесплатно на https://chatgpt.com (открою сейчас)."

    # Открываем ChatGPT, чтобы клиент мог войти/зарегистрироваться
    if command -v open >/dev/null 2>&1; then
      open "https://chatgpt.com" &>/dev/null &
      echo -e "   ${DIM}✓ Открыл chatgpt.com в браузере${NC}"
    elif command -v xdg-open >/dev/null 2>&1; then
      xdg-open "https://chatgpt.com" &>/dev/null &
      echo -e "   ${DIM}✓ Открыл chatgpt.com в браузере${NC}"
    fi
    ...
    explain "Сейчас запустится вход в ChatGPT." \
      "Появится ссылка (или код) — откройте её в браузере и войдите." \
      "После входа всё подключится автоматически, ничего вставлять не нужно."
